...
fix_object_segs()
{
	...
	for (i=0;i<=Highest_object_index;i++)
		...
		{
			//CTFC Begin
			//at start move flags to its homes regardles where map creator did put them
			if (Objects[i].type == OBJ_POWERUP)
				{
				if ((Objects[i].id == POW_FLAG_BLUE) && (Objects[i].segnum != goal_blue_cube) && (Objects[i].segnum > -1))
					{
					obj_relink(i, goal_blue_cube);
					compute_segment_center(&Objects[i].pos,&Segments[Objects[i].segnum]);
					t = update_object_seg(&Objects[i]);
					}
				if ((Objects[i].id == POW_FLAG_RED) && (Objects[i].segnum != goal_red_cube) && (Objects[i].segnum > -1))
					{	
					obj_relink(i, goal_red_cube);
					compute_segment_center(&Objects[i].pos,&Segments[Objects[i].segnum]);
					t = update_object_seg(&Objects[i]);	
					}
