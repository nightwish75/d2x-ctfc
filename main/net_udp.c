...
void net_udp_more_game_options ()
{
	...
	m[opt].type = NM_TYPE_TEXT; m[opt].text = "Packets per second (10 - 20)"; opt++;
	opt_packets=opt;
...
int net_udp_setup_game()
{
	...
	m[optnum].type = NM_TYPE_RADIO; m[optnum].text = "CTF Classic"; m[optnum].value=(Netgame.gamemode == NETGAME_CAPTURE_FLAG); m[optnum].group=0; opt.capture=optnum; optnum++;
